...
`viable-continuation-lean` is now a direct (git-pinned) dependency of `srrg-lean`, so
`ViableContinuation.Core.Viability` and `HasViability` are available for import.
-/
